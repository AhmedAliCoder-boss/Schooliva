...
export default async function TransportPage() {
  ...
  return <main className="students-shell">
    ...
    <div className="student-table-wrap" style={{ marginTop: 24 }}><table className="student-table"><thead><tr><th>Student</th><th>Route</th><th>Amount</th><th>Frequency</th><th>Due day</th></tr></thead><tbody>{(fees ?? []).length ? (fees ?? []).map((fee) => <tr key={String(fee.id)}><td>{String(fee.student_id)}</td><td>{String(fee.route_id)}</td><td>{formatCurrency(fee.amount)}</td><td>{String(fee.frequency)}</td><td>{String(fee.due_day)}</td></tr>) : <tr><td colSpan={5}><div className="student-empty"><h3>No transport fees</h3><p>Transport fee assignments will appear here.</p></div></td></tr>}</tbody></table></div>
  </main>;
}
