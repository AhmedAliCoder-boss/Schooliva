...
export async function requireStudentContext(action: "view" | "create" | "update" | "delete") {
  ...
  const { data: allowed } = await current.supabase.rpc("has_permission", { target_school_id: membership.school_id, target_resource: "students", target_action: action });
