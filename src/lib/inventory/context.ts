...
export async function requireInventoryContext(action: "manage" | "view") {
  ...
  const { data: allowed } = await current.supabase.rpc("has_permission", { target_school_id: membership.school_id, target_resource: "inventory", target_action: action });
