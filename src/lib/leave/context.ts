...
export async function requireLeaveContext(action: "view" | "manage" | "approve") {
  ...
  const { data: allowed } = await current.supabase.rpc("has_permission", { target_school_id: membership.school_id, target_resource: "leave", target_action: action });
