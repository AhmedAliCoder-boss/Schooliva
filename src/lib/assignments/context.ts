...
export async function requireAssignmentContext(action: "view" | "manage" | "submit" | "review") {
  ...
  const { data: allowed } = await current.supabase.rpc("has_permission", { target_school_id: membership.school_id, target_resource: "assignments", target_action: action });
