...
export async function requireAttendanceContext(action: "view" | "manage") {
  ...
  const { data: allowed } = await current.supabase.rpc("has_permission", { target_school_id: membership.school_id, target_resource: "students.attendance", target_action: action });
