...
export async function requirePeopleContext(resource: "teachers" | "staff", action: "view" | "manage") {
  ...
  const { data: allowed } = await current.supabase.rpc("has_permission", { target_school_id: membership.school_id, target_resource: resource, target_action: action });
